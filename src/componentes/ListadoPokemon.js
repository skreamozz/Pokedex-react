...
const limitBase = 6;
const urlBase =
  "https://pokeapi.co/api/v2/pokemon/?offset=0&limit=" + limitBase;
...
const ListadoPokemon = () => {
  ...
  const pedirPokemons = async (lista) => {
    let fetchs = [];
    const data = await lista.results.map((x) =>
      fetch(x.url).then((x) => x.json())
    );
  ...
  return (
    <Container fluid className="mt-4">
      <Paginacion
        handleClick={handlePaginacion}
        disabled={pokes ? false : true}
        paginaActual={PaginaActual}
        maximo={Listado.count / limitBase}
        limite={limitBase}
        offset={offset}
      />
          ...
          pokes && Listado.results ? (
            <CardColumns>
              {pokes.map((pokemon, key) => (
                <PokemonCard key={key} pokemonProp={pokemon} />
              ))}
            </CardColumns>
          ) : (
