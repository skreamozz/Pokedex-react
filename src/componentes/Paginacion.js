...
const Paginacion = React.memo(({handleClick, disabled, paginaActual,maximo, limite ,offset}) => {
    ...
    const llenarItems = useCallback(() => {
        ...
        for(let i = inicio; i <= limite + inicio ; i++){
            if(i >= maximo) break;
            let item = <Pagination.Item active = {(i+1 === paginaActual)} key={i} disabled = {disabled} onClick={handleClick(i+1)}>{i+1}</Pagination.Item>
            tempItems.push(item);
        }
        setItems(tempItems);
    ...
    return (
            ...
            <Pagination className= 'overflow-auto' >
                ...
                <Pagination.Ellipsis onClick = { handleClick((paginaActual + limite > maximo)? maximo : paginaActual + limite )} disabled = {disabled || paginaActual >= maximo}/>
                <Pagination.Next onClick={handleClick('siguiente')} disabled = {disabled || paginaActual >= maximo} />
                <Pagination.Last onClick={handleClick('ultimo')} disabled = {disabled || paginaActual >= maximo}/>
            </Pagination>
